...
class TestChandraBackend:
    ...
    def test_is_available_false_without_cuda(self):
        ...
        with patch("bddk_mcp.ocr.chandra._cuda_available", return_value=False):
            with patch("bddk_mcp.ocr.chandra._chandra_available", return_value=True):
    ...
    def test_is_available_false_without_chandra(self):
        ...
        backend = ChandraBackend()
        with patch("bddk_mcp.ocr.chandra._cuda_available", return_value=True):
            with patch("bddk_mcp.ocr.chandra._chandra_available", return_value=False):
                assert backend.is_available() is False
    ...
    def test_load_manager_propagates_model_name_to_chandra_settings(self, monkeypatch):
        ...
        monkeypatch.setattr(chandra, "CHANDRA_MODEL_NAME", "test-org/override-model")
