...
q = (
    "SELECT doc_id, MAX(total_pages) AS total_pages FROM document_chunks GROUP BY doc_id "
    "ORDER BY doc_id;"
)
proc = subprocess.run(
    ['docker', 'compose', 'exec', '-T', 'db', 'psql', '-U', 'bddk', '-d', 'bddk',
     '-t', '-A', '-F', '|', '-c', q],
...
# 2. Init MCP
call('initialize', {'protocolVersion':'2024-11-05','capabilities':{},'clientInfo':{'name':'validator','version':'0.0.1'}}, rid=1)
...
for doc_id, total_pages in doc_pages:
    for p in range(1, total_pages + 1):
        rid += 1
        try:
            out = call('tools/call', {'name':'get_bddk_document','arguments':{'document_id':doc_id,'page_number':p}}, rid=rid)
            txt = extract_text(out)
        except Exception as e:
            errored.append((doc_id, p, str(e)[:120])); continue
        if not txt:
            empty.append((doc_id, p)); continue
        if 'airlocked' in txt.lower():
            airlocked.append((doc_id, p)); continue
        ...
        if body.lstrip().startswith('Invalid page') or 'Invalid page' in body[:200]:
            invalid_page.append((doc_id, p)); continue
        passed += 1
