...
doc_ids = [l.strip() for l in proc.stdout.splitlines() if l.strip()]
...
if airlocked: print(f"  AIRLOCKED IDs: {airlocked[:20]}{'...' if len(airlocked)>20 else ''}")
if empty:     print(f"  EMPTY IDs:     {empty[:20]}{'...' if len(empty)>20 else ''}")
if errored:   print(f"  ERRORED:       {errored[:5]}")
if passed:
