...
def build_parser() -> argparse.ArgumentParser:
    ...
    migrate.add_argument("--db", help="PostgreSQL DSN; defaults to BDDK_DATABASE_URL")

    bootstrap = subparsers.add_parser(
        "bootstrap",
        help="Migrate, import the reviewed seed corpus, and validate readiness",
    )
    bootstrap.add_argument("--db", help="PostgreSQL DSN; defaults to BDDK_DATABASE_URL")
...
async def _migrate(dsn: str | None) -> None:
    from bddk_mcp.db_lifecycle import migrate_database

    await migrate_database(dsn=dsn)


async def _bootstrap(dsn: str | None, seed_dir: Path | None, force: bool) -> dict:
    ...
    result = await seed.import_seed(dsn=dsn, force=force)
    await assert_database_ready(dsn=dsn)
    return result
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.command in (None, "serve"):
        ...
        if args.command is None:
            args = argparse.Namespace(transport=None, host=None, port=None)
        _run_serve(args)
        return

    try:
        if args.command == "migrate":
            asyncio.run(_migrate(args.db))
            print("Database schema is ready.")
            return
        if args.command == "bootstrap":
            result = asyncio.run(_bootstrap(args.db, args.seed_dir, args.force))
            print(
                "Bootstrap complete: "
