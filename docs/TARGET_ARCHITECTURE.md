...
| Database lifecycle | Current through schema v7 | v4 is the canonical legal model, v5 adds corpus release/activation/epoch state, v6 adds least-privilege status resolution, and v7 adds a typed immutable retained-generation plane. Recovery evidence schema v2 inventories 51 managed objects, including the activation sequence; it also enforces six application DSN exclusions and verifies six restored LOGIN profiles. A retained synthetic two-cluster PG17 report proves exact schema-v7 repository restore equality (**bddk_mcp/migrations/; bddk_mcp/operations/recovery.py:72-142; docs/evidence/LOCAL_PG17_V7_RECOVERY_DRILL.md**). Bank PITR/RPO/RTO, custody, target-size/TLS/HBA/LOGIN execution and controlled backup-growth evidence remain open. |
...
| Database lifecycle and recovery | Partial at the historical checkpoint | At this superseded checkpoint the checksum ledger ended at v0004 and no second-cluster dump had run. The current overlay above supersedes it with schema v7 and retained synthetic two-cluster recovery evidence. Bank backup/PITR, RPO, RTO, custody, and target-environment acceptance remain unproved. |
