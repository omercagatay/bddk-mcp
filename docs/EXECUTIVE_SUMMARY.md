...
- **Confirmed database and corpus-publication boundary:** the checksum ledger now ends at **schema v6**: v4 supplies the owner-only canonical legal model and its attested 69-constraint/21-index subset, v5 supplies append-only corpus releases/activations plus a mutation epoch, and v6 supplies the least-privilege abstention-first legal-status resolver (**bddk_mcp/migrations/runner.py:22-37**). A strict publisher persists release identity, manifest and retrieval-profile hashes, freshness-policy evidence and a corpus-state fingerprint. Any later corpus mutation invalidates the active view; strict local-corpus calls take a same-release reader lease and fail closed if the release changes before completion (**bddk_mcp/migrations/v0005_corpus_release_publication.py:15-124,831-1056; bddk_mcp/corpus_serving.py:29-35,104-187**). OpenShift enables this strict mode; local research can explicitly leave it disabled.
- **Confirmed lifecycle and recovery boundary:** migration, ingestion bootstrap and release publication use separate identities and OpenShift Jobs. The release publisher independently regenerates deterministic derivatives and proves exact database membership before append-only activation (**bddk_mcp/ingest/seed.py:1207-1300; deploy/openshift/jobs/**). Recovery evidence schema v2 covers 29 managed relations plus the activation sequence, rejects reuse of all six application DSNs for recovery administration, verifies six restored LOGIN profiles, and compares the active release and sequence exactly (**bddk_mcp/operations/recovery.py:60-106,555-603,1305-1414**). This is repository capability, not retained bank PITR/RPO/RTO acceptance evidence.
- **Confirmed evaluation controls, but not release evidence:** Phase 2 validates the selected corpus manifest, reads the active release through the evaluated MCP session, requires exact manifest ID/SHA equality, executes all calls on that same session, and rechecks the release at the end (**benchmark/phase2_e2e.py:304-350,531-570,657-668**). Release trust now has four signed layers: measured corpus manifest, expert dataset, legal-curator Citation-pack attestation, and legal-release checkpoint over retained source/acquisition/page/excerpt evidence and signed predecessor history (**benchmark/expert_evaluation.py; benchmark/legal_release_evidence.py**). Four canonical signer identities must be separated. Even a passing cryptographic preflight reports `bank_authorization_verified: false` and `model_scores_authorized: false`, because expert-dataset execution is not implemented (**benchmark/release_preflight.py:89-124**).
...
- **Open external or product boundaries:** no real authoritative regulation family, named curator/reviewer evidence, real retained legal-release bundle, bank-owned latest-checkpoint anchor, immutable corpus generation rollback, expert-executed model result, named client/model certification, signed promoted image, or bank-applied IdP/CA/Route/CNI/egress/LOGIN/backup/PITR acceptance exists. Provision-to-control/workpaper knowledge remains future work.
...
### Publication is bound to an active release, but prior generations are not retained
...
The remaining limitation is different: the corpus tables are still mutable and prior corpus bytes are not retained as independently servable generations. There is no tested rollback or reactivation of a previous complete corpus image.
...
- immutable retained corpus generations and tested rollback/reactivation; atomic active-release publication itself is implemented;
...
3. **Release consistency is implemented, rollback is not.** PostgreSQL persists an atomic active identity and strict tool calls remain on one release, but prior immutable corpus generations are not retained for rollback.
...
Regenerate the 9,675 current-profile chunks, independently review the changed artifact, set approved numeric freshness objectives, collect the per-document measurement chain, sign the canonical manifest, keep the trust key separate, bootstrap through the ingestion identity, and activate it through the independent release publisher. Durable release identity is already implemented.

### 2. Set numeric operational targets and execute the full restore

Approve values and rolling windows in the delivered eight-metric operational-objectives contract, including availability, latency, freshness, RPO, RTO, alerts and evidence retention. Then execute the delivered recovery-v2 workflow between distinct PG17 clusters and retain sanitized evidence covering all 29 relations, the activation sequence and six identities.

### 3. Execute the bank integration controls, not just repository preflight

Apply the reviewed roles to the six actual application LOGIN profiles; validate HBA/TLS/`verify-full`; provision the separate corpus PVC and trust Secret; run migration → grants → ingestion bootstrap/reindex → independent release publication in order; validate RFC 9728 discovery and JWT behavior through the bank Route/IdP; and admit only the signed release-image digest.
...
The current repository has focused PostgreSQL privilege/lifecycle/publication tests, a dedicated actual-LOGIN/ACL CI contract, official MCP stdio/HTTP tests, package-install verification, recovery-v2 orchestration, and mandatory rendered OpenShift contract tests. No retained report in the repository proves a bank cluster, bank LOGINs, live upstream production operation, intended named clients/models, GPU workload, bank-sized migration, or bank-accepted backup/PITR drill. Whether an unretained local two-cluster drill has been executed is not treated as production evidence. No secret values were inspected or exposed.
