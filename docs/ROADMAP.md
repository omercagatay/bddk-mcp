...
| H2-02A/B corpus generations | Open beyond the v5 guard | Document/sections replace transactionally and chunks are visible only under a current content/profile publication record; mutation invalidates publication. H2-02A must first retain and measure sealed typed generations without changing serving. H2-02B then generation-binds all reads/caches and adds policy-authorized activation/reactivation; rollback remains open until H2-02B passes. |
...
| H2-02A | Immutable retained-generation staging and cost evidence | v5 records hashes and activation events over mutable tables but retains neither the bytes nor exact generation membership, so a safe rollback target does not exist. | Add an additive, DDL-only generation schema separating physical generation, governed release, seal, and activation identities; stage typed generation-qualified rows under the existing mutation lock; validate every v5 fingerprint relation, retrieval/citation integrity, and exact counts/hashes; seal append-only; label legacy v5 releases `unretained`; emit privacy-safe heap/index/TOAST/WAL and backup-growth evidence. Do not change serving or expose rollback in this slice. | H1-06, H2-01; N-01 policy-verification primitives; N-03 retention objectives | M | P0 | Injected failure after every stage leaves the active v5 release unchanged; two sealed generations coexist and reproduce exact typed rows/state hashes; sealed rows reject update/delete/truncate/conflicting insert; legacy releases are never represented as retained; storage evidence reconciles to PostgreSQL catalog sizes and leaks no text, paths, DSNs, or principals. |
...
H2-02A is an intentionally additive safety boundary, not partial evidence of
rollback. The tracked JSON artifacts total 24,377,264 bytes (23.25 MiB), and raw
...
source bytes. Capacity and hot-generation count must therefore use the measured
H2-02A report and N-03 retention decision, not this lower bound.
