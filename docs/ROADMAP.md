...
Repository work now provides schema v7, 15 public plus 14 operator tools, one active-release MCP resource, durable corpus release/activation with epoch invalidation, strict per-call serving leases, coordinated corpus writers, an immutable retained-generation mechanism, a recovery-v2 contract, separate OpenShift lifecycle identities/Jobs, same-session Phase 2 corpus binding, and a four-layer cryptographic evaluation preflight. These are repository controls, not bank acceptance.
...
|---|---|---|
| Horizon 0 | Runtime, packaging, profiles, lifecycle, logging, and canonical registry stabilized | Repair/review/re-sign the tracked artifacts; keep documentation generated/checked against 15/14/29 and schema v7. |
...
| Horizon 2 | Hybrid retrieval, Citation v1, legal model pilot, atomic active release, abstention-first status resolver, and H2-02A retained-generation mechanism implemented | Complete H2-02A controlled backup/capacity acceptance, then H2-02B generation-bound serving and separately authorized rollback/reactivation; also add one authoritative real family, page-derivation provenance, and measured Turkish retrieval acceptance. |
...
| R-02 | Synthetic repository execution complete; bank acceptance open | The retained 2026-07-16 report proves a 51-object schema-v7 `pg_dump`/`pg_restore` across two isolated local PG17 clusters. Numeric RTO/RPO, PITR, bank-size/custody evidence, and bank DBA approval remain open. |
...
| H2-02A | Immutable retained-generation staging and cost evidence — repository mechanism implemented, external acceptance partial | V7 now creates a safe immutable database rollback target for an active v5 release; controlled backup growth and bank retention/capacity approval remain open. | Retain the additive v7 schema: 17 typed generation-qualified relations; distinct generation/release/seal/activation identities; exact-state/profile-derived physical generation and seal with per-release many-to-one bindings; exact v5 state-hash reproduction; append-only guards; `legacy_v5_unretained`; publisher-only `retain-corpus-generation`; catalog, ACL, telemetry, exhaustive durable-stage failure injection, and 51-object recovery coverage. Measure backup growth with a controlled bank/DBA backup, approve retention count/capacity, and retain sanitized evidence. Do not change serving or expose rollback in this slice. | H1-06, H2-01; N-03 retention objectives; bank DBA/storage | S | P0 | Focused tests inject failure at each of the 17 retained-member copies plus inventory, seal, and release binding and prove the active release and all durable generation records remain unchanged. They also prove that different state/profile pairs produce two coexisting sealed generations, while differently governed releases over one exact state/profile share one physical generation/seal through distinct bindings; sealed rows reject mutation/truncate, legacy releases are not backfilled, and catalog storage sizes reconcile without content leakage. H2-02A closes only after a controlled backup records measured growth instead of `not_measured` and bank owners approve retention/capacity against unique physical generations. A reported WAL value is accepted only as `observed_cluster_interval_not_exclusive`, never exact generation attribution. |
...
| R-02 | Synthetic repository execution complete; bank acceptance open | Logical-restore orchestration and fail-closed checks exist, and the retained schema-v7 report proves an exact two-cluster synthetic run. Bank-size restore/PITR, custody, approved objectives, and DBA acceptance remain open. |
...
| R-08 | Manifest/import contract, v5 publication, and v7 retention complete; governed serving open | Bootstrap emits the identity and requires a distinct publisher; the publisher persists the release and activation together. V7 can bind and seal that release into an immutable retained generation. The tracked corpus is unsigned, unquantified, unmeasured, and its declared chunks drift from current regeneration; reads remain bound to mutable v5 serving tables and reactivation is absent. |
...
| N-04 | **Repeat and retain the two-cluster PostgreSQL 17 recovery drill at the bank boundary** | The synthetic repository-scale schema-v7 drill passed; prove the same recovery-v2 contract under approved target-size, TLS/HBA/LOGIN, custody and PITR conditions. | All 51 managed objects—including v7 retained members, seals, activation sequence, and active-release state—plus fingerprints, row counts, six application-DSN exclusions, and six LOGIN profiles match; elapsed backup/restore/recovery evidence is sanitized and meets N-03; controlled backup growth, destructive/PITR cases, and recovery cases are retained and bank-approved. This does not claim H2-02B application rollback. | P0 / M | `database`, `recovery`, `testing`, `p0` | N-03; bank-like PG17 pair; local baseline in `docs/evidence/LOCAL_PG17_V7_RECOVERY_DRILL.md` |
| N-05 | **Validate all bank database identities including release publisher** | Exercise actual bank-managed TLS/HBA/membership/ACL policy for migrator, ingestion, publisher, public, operator, and telemetry identities. | Every DSN uses `verify-full` and the approved CA; exact positive/negative SQL contracts pass under real LOGINs; publisher alone can publish/activate; runtime identities cannot mutate corpus/release state; DBA signs sanitized evidence. | P0 / M | `database`, `security`, `bank-acceptance`, `p0` | Bank DBA/CA/PG17 service |
