...
| Complete | Explicit database lifecycle and repository-level least privilege | Serving performs no DDL, seed import, synchronization, or embedding backfill. Checksum-verified v0001-v0004 migrations, explicit bootstrap, exact owner/target/TLS/ACL/catalog checks, and per-connection identity admission fail closed. Bootstrap is bound to the exact manifest-declared artifact paths, rejects undeclared reserved seed filenames, and accepts the quantified/measured/signature/key gates directly before opening a DB pool; standalone verification is not a trust handoff. Completion emits path-free manifest ID/SHA evidence, but does not persist it in PostgreSQL. The eleven v0004 legal-curation tables grant no direct runtime privilege; the public reader sees only the validated, authoritative, non-fixture citation view. Readiness attests the complete v0004 catalog digest for exactly 69 constraints and 21 indexes. The disposable PG17 transactional allow/deny and actual-LOGIN identity/ACL contracts executed locally and passed; bank-created principals and DBA execution remain external. Evidence: **bddk_mcp/ingest/seed.py; bddk_mcp/cli.py; bddk_mcp/migrations/**; **bddk_mcp/db_lifecycle.py**; **bddk_mcp/db_identity.py**; **bddk_mcp/catalog_integrity.py**; **deploy/postgres/**; **tests/test_seed.py; tests/test_cli.py; tests/test_postgres_role_assets.py**. Migration v0003 still refuses a populated corpus until its blocking backfill is explicitly approved. |
...
| Partial | Retrieval consistency and structured evidence | Document text/sections publish transactionally and chunks require the current document/profile publication. Six retrieval tools expose typed evidence. Citation v1 reconstructs one exact section only when the v0004 view proves validated occurrence/version, authoritative evidence, non-fixture artifact, canonical identities, exact section/provision hashes, an independently recomputed full normalized-document hash, and the frozen-whitespace normalized range against retained section text. `SourceBlob` binds content identity; `SourceArtifact` independently binds the blob, canonical URI, and acquisition timestamp. Text and structured citation identities match. An official MCP session against real PostgreSQL validates this path with a synthetic family. Evidence: **bddk_mcp/citations.py; bddk_mcp/store/doc_store.py; bddk_mcp/tools/structured_outputs.py; tests/test_citations.py; tests/test_legal_versions.py; tests/test_structured_retrieval_outputs.py**. Authoritative source bytes/true pages, curator/source authenticity, and real corpus legal mappings remain absent; this is not yet audit-grade evidence. |
...
| Partial | Packaging, observability, recovery, and deployment | Distribution/PG CI, safe logging/metrics, hardened OpenShift manifests, an offline acceptance harness, guarded populated-v2 rehearsal, a second-cluster logical-restore workflow, deterministic SBOM/unsigned provenance, complete-history secret scanning, and fresh High/Critical vulnerability policy now exist. OpenShift preflight hashes the exact Kustomize v5.8.1 binary and checks strict rendered-object/security inventories. The reviewed `bank-bootstrap` overlay passes the strict freshness/signature/key flags directly to bootstrap and mounts the approved corpus PVC separately from the read-only corpus-trust Secret; preflight renders and exact-inventory-checks that contract. Its egress matrix requires approved regulatory-source/proxy HTTPS for both public and operator runtimes—public has live institution/announcement/bulletin/update tools—while lifecycle stays DNS/PostgreSQL-only. Container evidence uses Buildx `--provenance=false --load`, then binds descriptor/manifest, config, loaded image, and Syft SBOM; repository SLSA provenance remains unsigned. Model manifest/runtime/Dockerfile pins must agree, and any applied pending exception leaves promotion ineligible. Evidence: **deploy/openshift-overlays/bank-bootstrap/**; **bddk_mcp/operations/recovery.py; bddk_mcp/openshift_acceptance.py; .github/workflows/supply-chain.yml; scripts/supply_chain_evidence.py; supply-chain/**. All eight live OpenShift gates, a full isolated restore, signed promotion, numeric SLO/RPO/RTO, and bank evidence retention remain unexecuted or unapproved. |
| Partial | Regulatory knowledge and audit-grade legal evidence | v0004 now models stable instruments, separate source blobs/acquisitions, versions, events/status assertions, provisions, occurrence validation, and append-only review decisions; a deterministic synthetic amendment-family fixture resolves validated as-of state or abstains. This is a technical pilot only. No authoritative real-family import, authenticated curator/reviewer authority, retained authoritative bytes/page reconstruction, or provision-to-control/workpaper layer exists. The ordinary ingestion identity cannot write legal-curation tables. Evidence: **bddk_mcp/regulatory/**; **bddk_mcp/migrations/v0004_canonical_legal_versions.py; tests/test_legal_versions.py**. |
| Partial | Corpus and expert-evaluation release governance | The scope validator distinguishes numeric freshness objectives from per-document measured event evidence. R09 release use requires three separately verified trust inputs: a signed corpus manifest with measured freshness, a separately signed expert dataset, and a separately signed legal-curator attestation over the exact validated Citation export; reusing the dataset signer for legal attestation is rejected. Evidence: **bddk_mcp/corpus_manifest.py; benchmark/expert_evaluation.py; tests/test_corpus_manifest.py; tests/test_expert_evaluation.py**. The checked-in corpus is unsigned/unmeasured and the 20-case set remains draft, unannotated, unadjudicated, unapproved, and unmapped to verified Citation v1 evidence. |
...
| Testing and evaluation | 3/5 | Broad unit/PostgreSQL 17/protocol/package/legal-pilot/recovery-policy/supply-chain-contract coverage exists. Full restore, live models, expert regulatory judgments, load, and bank OpenShift acceptance are missing. |
...
At the reviewed commit, `uv.lock` pinned MCP SDK **1.27.0**; it understood the 2025-11-25 protocol revision and both standard transports. The 2026-07-15 working tree instead locks **1.28.1**, whose `LATEST_PROTOCOL_VERSION` remains 2025-11-25. The repository's absence of resources/prompts is compliant. The following gaps describe the reviewed commit; the implementation overlay above records the later transport, authorization, schema, annotation, error, and protocol-test controls:
...
The 2026-07-15 implementation is a coherent engineering beta. In addition to the stabilized MCP/security/database foundation, it now has guarded recovery workflows, a PG17 contract, offline OpenShift evidence preflight, a repository supply-chain lane, corpus governance, a synthetic legal-version pilot, fail-closed Citation v1, a 20-case Turkish draft, and complete text-channel injection framing. It is still not ready for audit-grade legal reliance or bank production acceptance. The next priorities are full isolated restore and bank integration; authoritative legal-family/curator evidence with retained bytes/pages; independent expert annotation/adjudication; whole-corpus rollback; named-client/model certification; signed promotion; and measurable operations SLOs.
