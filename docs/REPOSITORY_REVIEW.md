...
| Partial | Packaging, recovery, OpenShift and operational objectives | Recovery evidence schema v2 now inventories 51 managed objects; that count includes the activation sequence, 17 retained member tables, generation inventory/seals/release binding, and the retention-status view. It rejects all six application DSNs for recovery administration, provisions/verifies six LOGIN profiles, and requires exact logical/catalog/identity equality (**bddk_mcp/operations/recovery.py:72-142; tests/test_recovery_workflows.py:472-514**). OpenShift separates schema migration, ingestion bootstrap and release publication into three Jobs and distinct ServiceAccounts/Secrets; generation retention currently remains a separate approved one-shot CLI operation, not an MCP surface or checked-in lifecycle Job (**deploy/openshift/serviceaccounts.yaml; deploy/openshift/jobs/**). A versioned eight-metric bank-on-prem OpenShift objectives contract exists, but every target/window is null and unapproved and production eligibility fails closed (**docs/decisions/operational-objectives.v1.yml; bddk_mcp/operational_objectives.py:333-501**). No retained bank PITR, controlled backup-growth measurement, cluster, approved-objective or signed-promotion evidence exists. |
