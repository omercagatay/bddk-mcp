...
| Complete | Application HTTP security baseline | A non-loopback listener requires exact Host and HTTPS Origin allowlists, a complete asymmetric JWT/JWKS profile, the correct public/operator scope, and explicit operator exposure. The composed FastMCP application exposes RFC 9728 protected-resource metadata at `/.well-known/oauth-protected-resource/mcp`, and a 401 challenge identifies that URL through `resource_metadata`. Token, byte, concurrency, and per-process rate bounds are enforced. Independent first-byte, inter-chunk, and total body deadlines now return 408 and release admission rather than allowing a slow sender to hold a slot indefinitely; the focused HTTP-security lane passed 61 tests. Fixed liveness/readiness routes disclose no corpus data. Evidence: **bddk_mcp/http_security.py; tests/test_http_security.py; tests/test_mcp_http_runtime.py; bddk_mcp/transport_tls.py; tests/test_server_tls.py**. Bank IdP registration/flows, ingress, CA, shared quotas, and ingress timeout acceptance remain external. |
| Complete | Explicit database lifecycle and repository-level least privilege | Serving performs no DDL, seed import, synchronization, embedding backfill, release publication, or generation retention. The checksum ledger ends at schema v8: v4 supplies eleven owner-controlled legal-curation tables and the validated-citation view, v5 supplies release/activation/epoch state, v6 supplies the security-definer legal-status resolver, v7 adds the isolated retained-generation plane, and v8 adds staged verification requests and one-time activation bindings. Legal-table mutation remains owner-only; v8 gives the release verifier the exact read-only access needed for publication evidence. Exact target/TLS/LOGIN/membership/ACL/catalog checks and per-connection admission fail closed. Schema owner, ingestion, release verifier, release publisher, public, operator and telemetry are distinct database profiles. The verifier can stage but not activate/retain; the publisher can activate by request ID and retain but cannot stage or directly publish claimed verification evidence. The v4 legal subset remains attested at 69 constraints and 21 indexes; v7 and v8 separately attest their exact schemas and ACLs. Evidence: **bddk_mcp/migrations/v0008_staged_corpus_releases.py; bddk_mcp/migrations/runner.py; bddk_mcp/db_identity.py; bddk_mcp/catalog_integrity.py; deploy/postgres/**. Focused evidence passed 59 PG17 migration/catalog tests, 118 application/role tests, and two actual-LOGIN tests. Bank-created principals, separation of the two release Secrets, HBA and DBA execution remain external. |
| Complete | Durable single-replica operator jobs and corpus coordination | Operator jobs persist privacy-safe state in `bddk_operator.operator_jobs`. A session-scoped job-admission lease and a distinct transaction-scoped corpus-mutation lock serve different purposes; sanctioned writers plus v8 staging and activation share the transaction key. Recovery handles abandoned running and cancel-requested work. Evidence: **bddk_mcp/corpus_coordination.py:1-40; bddk_mcp/migrations/v0008_staged_corpus_releases.py:209,439; bddk_mcp/jobs/postgres.py; bddk_mcp/jobs/manager.py; tests/test_bulk_write.py:118-231**. Execution still occurs in-process; the supplied OpenShift operator remains one `Recreate` replica and is not a distributed workflow engine. |
...
| Complete repository mechanism; governed release blocked | Staged active-corpus publication, strict serving, and retained rollback targets | V5 persists append-only release and activation evidence, maintains a statement-level mutation epoch over 17 corpus tables, and exposes an active view only when the latest activation epoch still equals current state. V8 remediates the publisher-credential verification bypass: `verify-and-stage-corpus-release` runs under a separate verifier identity and stages an expiring request bound to DB-computed state, epoch and profile plus verifier revision/image; `activate-corpus-release` supplies only that request ID under the publisher identity and atomically rejects reuse, expiry or state drift. The publisher cannot stage or execute the old direct routine. V7 can separately copy the same 17 typed relations into `bddk_retained`, reproduce the exact v5 state hash, seal the generation, and bind it without changing activation or serving. Evidence: **bddk_mcp/migrations/v0005_corpus_release_publication.py; bddk_mcp/migrations/v0007_retained_corpus_generations.py; bddk_mcp/migrations/v0008_staged_corpus_releases.py; bddk_mcp/corpus_publication.py; bddk_mcp/cli.py; tests/test_migrations.py; tests/test_corpus_publication.py**. A bank principal with both credentials or schema-owner authority can still collapse this separation. Immutable retention still is not rollback: reads/caches are not generation-qualified and no retained-generation reactivation exists until H2-02B. The tracked 8,286/9,675 drift continues to block a governed release. |
...
| Partial | Packaging, recovery, OpenShift and operational objectives | Recovery-v2 inventories 53 managed objects, rejects seven application DSNs for recovery administration, provisions/verifies seven LOGIN profiles, and covers both v8 staged-release relations. A retained local two-cluster PostgreSQL 17 execution proves schema v8 equality for all 53 objects, seven identities, two request/binding rows and two retained generations, including exact logical fingerprint and active release (**bddk_mcp/operations/recovery.py; tests/test_recovery_workflows.py; docs/evidence/LOCAL_PG17_V8_RECOVERY_DRILL.md**). OpenShift separates schema migration, ingestion bootstrap, verifier-side verify/stage, and publisher-side activation into four Jobs and distinct ServiceAccounts/Secrets; the activator receives neither corpus PVC nor trust key. Focused OpenShift integration passed 75 tests. Generation retention remains a separate approved one-shot CLI operation. A versioned eight-metric bank-on-prem contract exists, but every target/window is null and unapproved. No bank PITR, Secret/RBAC separation proof, controlled backup-growth measurement, target-size/TLS/HBA cluster run, approved-objective or signed-promotion evidence exists. |
...
The 2026-07-16 implementation is a coherent engineering beta. In addition to the stabilized MCP/security/database foundation, it has a 29-tool registry, one active-release resource, schema v8, verifier-staged/request-ID-only corpus activation with mutation epochs, strict per-tool release binding, immutable 17-relation retained-generation targets, a recovery-v2 contract and retained local two-cluster execution over 53 managed objects and seven identities, four OpenShift lifecycle Jobs, bounded HTTP body-read deadlines, a four-layer legal/evaluation verifier, and an eight-metric operational decision contract. The project is still not ready for audit-grade legal reliance or bank production acceptance: the tracked 8,286-row chunk artifact regenerates as 9,675 rows and cannot pass strict publication; no real authoritative family or retained legal-release bundle exists; model-score execution and bank authorization remain absent; generation-bound serving/authorized rollback, bank verifier/publisher credential separation, controlled backup growth, approved retention/capacity targets, bank PITR/TLS/HBA/target-size recovery evidence, signed promotion and named-client/model certification remain open.
