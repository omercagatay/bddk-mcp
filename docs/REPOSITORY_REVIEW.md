...
| Partial technical capability | Regulatory knowledge and audit-grade legal evidence | V4 models stable instruments, separate source blobs/acquisitions, versions, events/status assertions, provisions, occurrences and append-only review decisions; v6 resolves validated as-of status or abstains. A separate legal-release verifier re-hashes retained authoritative source bytes, acquisition records, page text/mappings and Citation excerpts, verifies signed predecessor history, and requires a caller-supplied latest-checkpoint hash (**benchmark/legal_release_evidence.py:353-616**). No real non-fixture family, retained evidence bundle, authenticated curator/reviewer, bank-owned latest anchor, or provision-to-control/workpaper layer exists. |
| Partial, deliberately non-release | Corpus and expert-evaluation release governance | Release use requires four signed evidence layers: measured corpus manifest, expert dataset, legal-curator attestation over the exact validated Citation pack, and legal-release checkpoint over retained evidence/history. Canonical signer fingerprints must be separated. A passed cryptographic preflight still states `bank_authorization_verified: false` and `model_scores_authorized: false`; expert-dataset execution is not implemented (**benchmark/expert_evaluation.py; benchmark/legal_release_evidence.py; benchmark/release_preflight.py:89-124**). The checked-in corpus is unsigned/unmeasured and chunk-stale; the 20-case set remains draft, unannotated, unadjudicated, unapproved and unmapped to verified real evidence. |
