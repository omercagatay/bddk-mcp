...
| Complete | Explicit database lifecycle and repository-level least privilege | Serving performs no DDL, seed import, synchronization, embedding backfill, release publication, or generation retention. The checksum ledger ends at schema v7: v4 supplies eleven owner-only legal-curation tables and the validated-citation view, v5 supplies release/activation/epoch state, v6 supplies the security-definer legal-status resolver, and v7 adds the isolated retained-generation plane. Exact target/TLS/LOGIN/membership/ACL/catalog checks and per-connection admission fail closed. Schema owner, ingestion, release publisher, public, operator and telemetry are distinct database profiles. The v4 legal subset remains attested at 69 constraints and 21 indexes; v7 separately attests its exact retained schema, constraints, indexes, triggers, routines, and view. Evidence: **bddk_mcp/migrations/runner.py; bddk_mcp/db_lifecycle.py; bddk_mcp/db_identity.py; bddk_mcp/catalog_integrity.py:450-580; deploy/postgres/**. Disposable PG17 contracts are repository evidence; bank-created principals, HBA and DBA execution remain external. |
...
| Complete repository mechanism; governed release blocked | Active-corpus publication, strict serving, and retained rollback targets | V5 persists append-only release and activation evidence, maintains a statement-level mutation epoch over 17 corpus tables, and exposes an active view only when the latest activation epoch still equals current state. V7 can atomically copy those same 17 typed relations into `bddk_retained`, reproduce the exact v5 state hash, seal the generation, and create a binding for the active governed release without changing activation or serving. Physical generation, governed release, seal, and activation identities remain distinct. Generation/seal identity is derived from exact corpus state and retrieval profile, so multiple governed releases over that same pair share one physical generation and seal through distinct per-release bindings. The one-shot `bddk-mcp retain-corpus-generation` command is release-publisher-only and is not an MCP tool. Evidence: **bddk_mcp/migrations/v0005_corpus_release_publication.py; bddk_mcp/migrations/v0007_retained_corpus_generations.py:1-8,240-367,461-846; bddk_mcp/corpus_generations.py:1-6; bddk_mcp/cli.py:294-345; tests/test_corpus_publication.py; tests/test_cli.py**. This creates immutable rollback targets, not rollback: v5 reads and caches are not generation-qualified and no activation/reactivation path exists until H2-02B. Any v5 release without a v7 binding remains explicitly `legacy_v5_unretained`; controlled backup growth and bank retention/capacity approval remain open. The tracked manifest still declares 8,286 chunks but current-profile regeneration produces 9,675, so strict publication refuses it until regeneration, independent review and re-signing (**seed_data/corpus_scope.yml:31-40; bddk_mcp/ingest/seed.py:383-405,657-698**). |
...
| Partial | Packaging, recovery, OpenShift and operational objectives | Recovery evidence schema v2 inventories 51 managed objects; that count includes the activation sequence, 17 retained member tables, generation inventory/seals/release binding, and the retention-status view. It rejects all six application DSNs for recovery administration, provisions/verifies six LOGIN profiles, and requires exact logical/catalog/identity equality. A retained synthetic two-cluster PostgreSQL 17 report now proves that complete schema-v7 repository path (**bddk_mcp/operations/recovery.py:72-142; tests/test_recovery_workflows.py:472-514; docs/evidence/LOCAL_PG17_V7_RECOVERY_DRILL.md**). OpenShift separates schema migration, ingestion bootstrap and release publication into three Jobs and distinct ServiceAccounts/Secrets; generation retention remains a separate approved one-shot CLI operation, not an MCP surface or checked-in lifecycle Job (**deploy/openshift/serviceaccounts.yaml; deploy/openshift/jobs/**). A versioned eight-metric bank-on-prem OpenShift objectives contract exists, but every target/window is null and unapproved and production eligibility fails closed (**docs/decisions/operational-objectives.v1.yml; bddk_mcp/operational_objectives.py:333-501**). No retained bank PITR, controlled backup-growth measurement, target-cluster, approved-objective or signed-promotion evidence exists. |
