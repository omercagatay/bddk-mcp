...
| Complete | Explicit lifecycle and PostgreSQL authorization | `serve` is read-only with respect to schema/corpus lifecycle. The checksum ledger ends at v7: v4 supplies eleven owner-only legal-curation tables and the validated-citation view, v5 supplies corpus release/activation/epoch state, v6 supplies the security-definer legal-status resolver, and v7 supplies isolated retained-generation storage. Exact target/TLS/ACL/catalog/connection identities are checked for schema-owner, ingestion, release-publisher, public, operator and telemetry profiles. The v4 legal subset remains attested at 69 constraints and 21 indexes; the v7 retained schema has a separate exact catalog digest. Evidence: **bddk_mcp/migrations/runner.py; bddk_mcp/db_identity.py; bddk_mcp/db_lifecycle.py; bddk_mcp/catalog_integrity.py:450-580; deploy/postgres/**. Bank LOGIN/HBA/DBA acceptance remains external. |
...
| Complete mechanism; governed release blocked | Active corpus publication, strict serving, and immutable retained snapshots | V5 appends release/activation evidence and increments one statement-level epoch on mutations to 17 tracked corpus tables. The security-barrier active view exists only while the latest activation epoch equals current state. V7 adds 17 typed, generation-qualified mirror relations plus generation, inventory, seal, and release-binding metadata. Under the schema-migration and corpus-mutation locks, its publisher-only facade copies one active state, reproduces the exact v5 fingerprint, seals it, and leaves the v5 active view unchanged. Physical generation, governed release, seal, and activation are separate identities. Generation/seal identity is exact-state/profile-derived, so multiple governed releases over that same pair keep distinct release bindings to one physical generation and seal; any v5 release without a v7 binding is surfaced as `legacy_v5_unretained`. Evidence: **bddk_mcp/migrations/v0005_corpus_release_publication.py; bddk_mcp/migrations/v0007_retained_corpus_generations.py:21-41,240-367,461-846; tests/test_corpus_publication.py**. The tracked artifact declares 8,286 chunks but regenerates as 9,675 and is rejected by strict publication. Retention is not generation-bound serving or rollback; H2-02B remains open. |
| Complete repository boundary; capacity acceptance partial | Administrative retention facade | `bddk-mcp retain-corpus-generation --expected-release-id …` uses only the release-publisher database profile, executes one atomic transaction, and emits content-free receipt and storage evidence. It is absent from both MCP registries. Public, ingestion, operator, and telemetry profiles have no `bddk_retained` schema usage or direct table privileges; the publisher receives only the retention-status view and two security-definer functions. The CLI reports heap/auxiliary/TOAST/index sizes and, when the post-commit observation is available, a cluster WAL interval labelled non-exclusive; otherwise WAL remains `not_measured`. Backup growth remains `not_measured` until a controlled bank/DBA backup supplies it, and capacity must be approved against unique physical generations rather than release-binding count. Evidence: **bddk_mcp/cli.py:119-127,294-345; bddk_mcp/corpus_generations.py:117-133; bddk_mcp/db_identity.py:220-251; deploy/postgres/02_grants.sql; tests/test_cli.py; tests/test_postgres_role_assets.py**. |
...
| Partial | OpenShift and recovery topology | The starter separates public/operator workloads and three lifecycle Jobs: schema-owner migration, ingestion bootstrap/reindex, and independent release publication, each with reviewed ServiceAccounts and Secrets. Retention is presently an additional approved one-shot CLI action, not a public/operator MCP call or included OpenShift Job. Recovery evidence schema v2 fingerprints 51 managed objects, with all v7 retained members/seals and the activation sequence included in that count; it rejects six runtime DSNs for admin use, verifies six restored LOGIN profiles, and requires exact active-release/sequence/logical equality. A retained synthetic two-cluster PG17 run proves that repository path. Evidence: **deploy/openshift/serviceaccounts.yaml; deploy/openshift/jobs/**; **bddk_mcp/operations/recovery.py:72-142; tests/test_recovery_workflows.py:472-514; docs/evidence/LOCAL_PG17_V7_RECOVERY_DRILL.md**. Eight live-environment gates, bank CNI enforcement, namespace execution, retained bank restore/PITR evidence and approved RPO/RTO remain open. |
