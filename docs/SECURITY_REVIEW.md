...
| Database and legal-curation privilege boundary | Complete as repository contract | Exact target, `verify-full` transport, LOGIN/effective membership, ACL provenance, per-connection admission, catalog integrity, and checksum migrations fail closed through v0004. Disposable PG17 transactional allow/deny and actual-LOGIN identity/ACL contracts executed locally and passed. All eleven v0004 legal-curation tables are owner-only, and readiness requires the exact catalog digest for 69 constraints and 21 indexes. From that legal layer the public reader can select only the security-barrier/definer `public.regulatory_validated_section_citations` view; public, ingestion, and operator identities have no direct base-table privilege (**bddk_mcp/db_identity.py; bddk_mcp/db_lifecycle.py; bddk_mcp/catalog_integrity.py; bddk_mcp/migrations/v0004_canonical_legal_versions.py; deploy/postgres/**). Bank curator/reviewer identities, bank LOGINs, and DBA execution evidence remain external. |
| Citation and legal-claim integrity | Partial technical pilot | `SourceBlob` provides content identity while `SourceArtifact` separately identifies an acquisition. The validated view excludes unvalidated, non-authoritative, fixture, cross-instrument, and hash-inconsistent mappings, binds the frozen-whitespace range to exact retained section text, and Citation v1 rechecks canonical identity and stored/recomputed hashes. An official MCP session against real PostgreSQL proves the synthetic contract path (**bddk_mcp/citations.py; bddk_mcp/store/doc_store.py; tests/test_citations.py; tests/test_legal_versions.py**). Artifact content remains a declaration rather than retained authoritative bytes/pages, and the only family fixture is synthetic. No real legal currentness, curator authority, or source authenticity is proved. |
...
| Acquisition and corpus scope | Partial | Exact approved HTTPS hosts, redirect/public-address validation, bounded response/retry/archive behavior, hardened XML parsing, and per-document publication checks fail closed. Bootstrap reads only manifest-role paths, rejects undeclared reserved filenames, and enforces requested quantified/measured/signature gates with a separately supplied trust key before opening a DB pool; a prior verifier run confers no authority. It emits path-free manifest ID/SHA operator evidence, which is not yet persisted in PostgreSQL (**bddk_mcp/ingest/seed.py; bddk_mcp/cli.py; tests/test_seed.py; tests/test_cli.py**). The 318-document scope remains unsigned, unquantified, unmeasured, and explicitly non-exhaustive. Bank egress, malware/source authenticity, whole-corpus identity/rollback, and private-corpus/tenant policy remain open. |
| Recovery and upgrade integrity | Partial repository workflows | A guarded v2-to-v4 rehearsal proves default refusal, reindex/readiness, actual-content fingerprints, and bounded PostgreSQL subprocess timeout/cleanup. A logical restore workflow re-applies roles and checks identities/catalog/fingerprints (**bddk_mcp/operations/recovery.py; tests/test_recovery_workflows.py; docs/RECOVERY_DRILLS.md**). The full `pg_dump` into a second isolated cluster has not been run; bank PITR, backup custody, RPO/RTO, and restore acceptance are unproved. |
...
| Evaluation integrity | Partial, deliberately non-release | The 20-case/eight-domain Turkish dataset is checksum/corpus-bound, but all cases remain draft and legal currentness/Citation mapping is unverified. Release validation requires three separately verified trust inputs: signed corpus with measured freshness evidence, separately signed dataset, and separately signed exact validated-Citation export/attestation under a distinct legal-curator key; dataset/legal signer reuse is rejected. Complete reconstruction, minimum coverage, independent annotation, adjudication, approval, and chronology also fail closed (**benchmark/expert_evaluation.py; benchmark/expert_evaluation_draft.yml**). It cannot support a model or security claim today. |

Current security maturity remains **3/5**. Cross-document ratings remain overall **3/5**, production readiness **2/5**, MCP **4/5**, retrieval **3/5**, testing/evaluation **3/5**, and documentation **4/5**. Production approval remains blocked by bank-applied identity/network/CA/egress controls, bank LOGIN/DBA and curator evidence, a completed isolated restore, signed promotion, source-artifact authenticity, live-model injection evaluation, numeric SLO/RPO/RTO, and real cluster validation.
...
Repository upgrade documentation requires a restorable backup and size-matched rehearsal before dangerous migration approval. The current working tree adds guarded populated-v2-to-v4 rehearsal and logical-restore workflows, content/vector fingerprints, identity/catalog/readiness checks, and bounded PostgreSQL subprocess cleanup. Focused automated tests cover those contracts. A complete dump into a second isolated cluster has not been executed, so no bank-accepted evidence yet covers:

- managed backup/PITR or a completed isolated logical restore;
- measured and approved RPO/RTO;
- authoritative source-artifact recovery;
- active-generation rollback, because whole-corpus generations do not exist;
- key/config recovery;
...
- actual bank PostgreSQL LOGINs, memberships and ACL provenance; TLS/HBA policy; shared-cluster role names; database size; backups; and restore/upgrade results. Repository role, target-database, `verify-full`, per-connection identity, catalog, v0004, guarded v2-to-v4, and logical-restore contracts exist, but the opt-in actual-LOGIN and full isolated-restore proof have not run in that environment;
...
- trusted signing keys and approval identities for the currently unsigned corpus manifest and expert dataset, plus numeric corpus detection/publication/age objectives;
