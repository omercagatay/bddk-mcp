...
| Recovery and upgrade integrity | Partial repository workflows | A guarded populated-v2-to-current-schema rehearsal proves default refusal, reindex/readiness, actual-content fingerprints, and bounded PostgreSQL subprocess timeout/cleanup. Recovery evidence schema v2 now covers 51 managed objects; that count includes the activation sequence, 17 retained member tables, and v7 generation/inventory/seal/release-binding/status records. It rejects all six application DSNs as recovery administration and verifies six restored LOGIN profiles (**bddk_mcp/operations/recovery.py:72-142; tests/test_recovery_workflows.py:472-514; docs/RECOVERY_DRILLS.md**). This is a repository workflow/contract; retained bank PITR, backup custody, approved RPO/RTO, and bank restore acceptance remain unproved. |
