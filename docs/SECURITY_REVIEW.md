...
| Database and legal-curation privilege boundary | Complete as repository contract | Exact target, `verify-full` transport, LOGIN/effective membership, ACL provenance, per-connection admission, catalog integrity, and checksum migrations fail closed through v0007. Disposable PG17 transactional allow/deny and actual-LOGIN identity/ACL contracts executed locally and passed. All eleven v0004 legal-curation tables are owner-only, and readiness still attests the exact v4 digest of 69 constraints and 21 indexes; v5 adds attested append-only corpus release/activation state, v6 an attested SECURITY DEFINER abstention-first status resolver, and v7 an exact attested retained schema. From the legal layer the public reader can select only the security-barrier/definer validated-Citation view and execute the narrowly granted resolver; public, ingestion, and operator identities have no direct legal-table privilege. All runtime profiles also lack `bddk_retained` schema usage/direct-table privileges (**bddk_mcp/db_identity.py; bddk_mcp/catalog_integrity.py:450-580; bddk_mcp/migrations/; deploy/postgres/**). Bank curator/reviewer identities, bank LOGINs, and DBA execution evidence remain external. |
| Retained-generation authorization and integrity | Complete repository mechanism; rollback and capacity acceptance open | V7 copies the exact 17-table active v5 state under ordered advisory/table locks, reproduces its v5 state hash, seals typed rows, and creates a per-release binding. Physical generation, release, seal, and activation identities cannot be substituted for one another. Generation/seal identity is exact-state/profile-derived: multiple governed releases over that same pair legitimately share one physical generation and seal through distinct bindings, preventing governance identity from multiplying storage. Function-local `TimeZone=UTC`, `DateStyle=ISO, YMD`, `IntervalStyle=postgres`, `bytea_output=hex`, and `extra_float_digits=3` make retained-row and v5/retained-state hashes independent of caller formatting. Historical release hashes are not rewritten; a mismatching active pre-v7 release makes the v7 migration fail before replacing the hash function and requires reviewed canonical revalidation/publication through an exact publication-only identity/catalog path on the unchanged v5 or v6 schema, not ledger mutation or fabricated backfill. Serving, retention, and other workload admission remain v7-only. The release publisher receives only the retention-status view and the `retain_active_corpus_generation`/storage facades; it has no direct retained-table privilege. The CLI is absent from both MCP registries and never activates or serves a generation. V5 releases without a v7 binding are `legacy_v5_unretained`, and mutation/truncate/catalog-tamper tests fail closed (**bddk_mcp/migrations/v0007_retained_corpus_generations.py; bddk_mcp/corpus_generations.py; bddk_mcp/cli.py; bddk_mcp/db_identity.py; tests/test_corpus_publication.py; tests/test_catalog_integrity.py; tests/test_postgres_role_assets.py**). Its transaction-local `lock_timeout=30s` and `statement_timeout=30min` bound contention/work; timeout is atomic failure. The optional WAL baseline uses a savepoint and the endpoint is best-effort after commit, so measurement failure yields `not_measured` rather than undoing durable success; any value remains a non-exclusive cluster interval. H2-02B generation-bound reads and policy-authorized reactivation remain open. Backup growth is `not_measured`, and bank retention/capacity approval against unique physical generations is absent. |
...
| Acquisition and corpus scope | Partial | Exact approved HTTPS hosts, redirect/public-address validation, bounded response/retry/archive behavior, hardened XML parsing, and per-document publication checks fail closed. Bootstrap verifies manifest-role paths and policy; a distinct publisher revalidates and persists an append-only active release with manifest/retrieval-profile/corpus-state identity, while a mutation epoch invalidates stale activation (**bddk_mcp/ingest/seed.py; bddk_mcp/corpus_publication.py; bddk_mcp/migrations/v0005_corpus_release_publication.py**). V7 can retain the database fields behind that state, but it does not acquire missing external authoritative source bytes or prove source authenticity. The 318-document scope remains unsigned, unquantified, unmeasured, non-exhaustive, and its declared 8,286 chunks drift from 9,675 current-profile rows, so strict publication refuses it. Bank egress, malware/source authenticity, H2-02B rollback, and private-corpus/tenant policy remain open. |
| Recovery and upgrade integrity | Repository execution complete; bank acceptance open | A guarded populated-v2-to-current-schema rehearsal proves default refusal, reindex/readiness, actual-content fingerprints, and bounded PostgreSQL subprocess timeout/cleanup. Recovery evidence schema v2 covers 51 managed objects; that count includes the activation sequence, 17 retained member tables, and v7 generation/inventory/seal/release-binding/status records. It rejects all six application DSNs as recovery administration and verifies six restored LOGIN profiles. A retained synthetic two-cluster PG17 report proves exact source/restore identity, catalog, locale, sequence, active-release and logical-fingerprint equality for schema v7 (**bddk_mcp/operations/recovery.py:72-142; tests/test_recovery_workflows.py:472-514; docs/evidence/LOCAL_PG17_V7_RECOVERY_DRILL.md**). Bank PITR, backup custody, target-size/TLS/HBA/LOGIN execution, approved RPO/RTO, and bank restore acceptance remain unproved. |
...
| corpus_publisher | activate a governed release and retain its sealed generation; no raw corpus writes |
