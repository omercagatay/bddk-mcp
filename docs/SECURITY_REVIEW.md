...
|---|---|---|
| Remote application identity and request boundary | Complete at application boundary | Non-loopback startup fails closed without exact Host and HTTPS Origin allowlists, complete asymmetric JWT/JWKS verification, and the profile scope. The composed FastMCP application publishes RFC 9728 protected-resource metadata at `/.well-known/oauth-protected-resource/mcp`, and an unauthenticated 401 challenge supplies that URL through `resource_metadata`. Body size, rate, and concurrency admission are bounded per process. Independent first-byte, inter-chunk, and total body-read deadlines return 408 and release the slot, closing the slow-body starvation path; 61 focused HTTP-security tests passed (**bddk_mcp/http_security.py; tests/test_http_security.py; tests/test_mcp_http_runtime.py**). The repository additionally supports an explicit `BDDK_HTTP_ALLOW_UNAUTHENTICATED` opt-in that serves a non-loopback public read-only bind without bearer authentication; the fail-closed default with the opt-in unset is unchanged. The opt-in refuses to combine with any configured `BDDK_JWT_`-prefixed setting (startup names the offending variables), is refused outright for a non-loopback operator profile regardless of `BDDK_OPERATOR_REMOTE_ENABLED` — operator tools stay authenticated or loopback-only by construction — and advertises no OAuth discovery (no `WWW-Authenticate` challenge; 404 on both well-known routes). Host/Origin admission and the body, concurrency, and rate limits still apply, and the per-process rate limiter becomes the primary abuse control; its client key is governed by `BDDK_HTTP_TRUSTED_PROXY_HOPS` (default `0`: ASGI socket peer only, `X-Forwarded-For` ignored; `n` trusted hops: the `n`-th forwarded entry from the right, with anything unusable degrading to a shared `unknown` bucket rather than the socket peer). This is an application boundary, not shared ingress enforcement or proof of bank OAuth client registration/flows, TLS, IdP integration, or ingress timeout policy. |
...
| Database and legal-curation privilege boundary | Complete as repository contract | Exact target, `verify-full` transport, LOGIN/effective membership, ACL provenance, per-connection admission, catalog integrity, and checksum migrations fail closed through v0008. All eleven v0004 legal-curation tables are owner-controlled and mutation remains owner-only; v8 grants only the release verifier the exact read-only access needed to recompute publication evidence. Readiness still attests the exact v4 digest of 69 constraints and 21 indexes; v5 adds release/activation state, v6 a least-privilege resolver, v7 the retained schema, and v8 the staged-release request/binding plane. Public, ingestion, operator, publisher, and telemetry identities have no direct legal-table access. The verifier cannot activate/retain, and the publisher cannot stage or call the old direct publication routine. All runtime profiles lack direct `bddk_retained` table privileges (**bddk_mcp/migrations/v0008_staged_corpus_releases.py; bddk_mcp/db_identity.py; bddk_mcp/catalog_integrity.py; deploy/postgres/**). Focused evidence passed 59 PG17 migration/catalog tests, 118 application/role tests, and two actual-LOGIN tests. Bank curator/reviewer identities, issuance and RBAC separation of verifier/publisher credentials, HBA, and DBA execution remain external. |
| Release verification and activation separation | Repository-remediated; bank custody open | The former direct publisher path trusted caller-supplied claims after Python verification, so a stolen publisher credential could bypass signature/governance checks at the database routine. Schema v8 replaces the supported flow with an expiring request staged under `bddk_release_verifier`, bound to DB-computed state/epoch/profile and verifier revision/image, followed by one-time request-ID-only activation under `bddk_release_publisher`. Activation atomically rechecks readiness, expiry, reuse and drift; the two roles have no membership in one another and the publisher cannot execute the old direct routine (**bddk_mcp/migrations/v0008_staged_corpus_releases.py; bddk_mcp/corpus_publication.py; bddk_mcp/db_identity.py; deploy/postgres/02_grants.sql; tests/test_migrations.py; tests/test_corpus_publication.py**). This closes the repository-level publisher-only bypass. A principal with both credentials or schema-owner authority can still forge the end-to-end decision, so bank Secret/RBAC/rotation/audit separation remains a High deployment requirement. |
| Retained-generation authorization and integrity | Complete repository mechanism; rollback and capacity acceptance open | V7 copies the exact 17-table active v5 state under ordered advisory/table locks, reproduces its v5 state hash, seals typed rows, and creates a per-release binding. Physical generation, release, seal, request, and activation identities cannot be substituted for one another. Generation/seal identity is exact-state/profile-derived: multiple governed releases over that same pair legitimately share one physical generation and seal through distinct bindings. Function-local formatting settings make retained-row and state hashes caller-format independent. Historical release hashes are not rewritten; a mismatching active pre-v7 release requires the reviewed compatibility remediation rather than ledger mutation or fabricated backfill. Retained-generation semantics remain defined by v7, while current workload admission requires v8. The verifier has no retained-schema access. The publisher receives only active/retention-status views plus the narrow activate, retain, and storage-inspection facades; it has no raw corpus, legal-evidence, or direct retained-table privilege. The retention CLI is absent from both MCP registries and never activates or serves a generation. V5 releases without a v7 binding are `legacy_v5_unretained`, and mutation/truncate/catalog-tamper tests fail closed (**bddk_mcp/migrations/v0007_retained_corpus_generations.py; bddk_mcp/migrations/v0008_staged_corpus_releases.py; bddk_mcp/corpus_generations.py; bddk_mcp/db_identity.py; tests/test_corpus_publication.py; tests/test_catalog_integrity.py; tests/test_postgres_role_assets.py**). Transaction timeouts bound retention work; optional WAL remains a non-exclusive observed interval or `not_measured`. H2-02B generation-bound reads and policy-authorized reactivation remain open. Backup growth is `not_measured`, and bank retention/capacity approval is absent. |
...
| Acquisition and corpus scope | Partial | Exact approved HTTPS hosts, redirect/public-address validation, bounded response/retry/archive behavior, hardened XML parsing, and per-document publication checks fail closed. Bootstrap verifies manifest-role paths and policy; the release verifier independently revalidates and stages state-bound evidence, then the publisher activates only that request ID. A mutation epoch invalidates stale activation (**bddk_mcp/ingest/seed.py; bddk_mcp/corpus_publication.py; bddk_mcp/migrations/v0008_staged_corpus_releases.py**). V7 can retain database fields behind that state, but it does not acquire missing external authoritative source bytes or prove source authenticity. The 318-document scope remains unsigned, unquantified, unmeasured, non-exhaustive, and its declared 8,286 chunks drift from 9,675 current-profile rows, so verification/staging refuses it. Bank egress, malware/source authenticity, H2-02B rollback, and private-corpus/tenant policy remain open. |
| Recovery and upgrade integrity | Current local execution complete; bank acceptance open | A guarded populated-v2-to-current-schema rehearsal proves default refusal, reindex/readiness, actual-content fingerprints, and bounded PostgreSQL subprocess timeout/cleanup. Recovery covers 53 managed objects, including both v8 staged-release relations, rejects all seven application DSNs as recovery administration, and verifies seven restored LOGIN profiles. A retained synthetic two-cluster PG17 execution proves exact schema-v8 source/restore equality for all 53 objects, two request/bindings, two retained generations, logical fingerprint and active-release identity (**bddk_mcp/operations/recovery.py; tests/test_recovery_workflows.py; docs/evidence/LOCAL_PG17_V8_RECOVERY_DRILL.md; docs/evidence/local-pg17-v8-restore-2026-07-16.json**). Bank PITR, backup custody, target-size/TLS/HBA/LOGIN execution, approved RPO/RTO, and bank restore acceptance remain unproved. |
| OpenShift deployment boundary | Partial repository preflight | The non-root starter separates planes and four lifecycle Jobs: migrate, bootstrap, verifier-side verify/stage, and publisher-side activation. Verifier and activator use separate ServiceAccounts/Secrets/DSNs; the activation Job receives no corpus PVC or trust key. The acceptance harness performs a checksum-pinned Kustomize build and enforces exact resource, policy, command, secret, volume, and restricted-security-context inventories; 75 focused integrated tests passed (**deploy/openshift/jobs/; deploy/openshift/serviceaccounts.yaml; bddk_mcp/openshift_acceptance.py; tests/test_openshift_acceptance.py; tests/test_openshift_manifests.py**). Its success still leaves eight live gates `not_run` and cannot prove that bank RBAC prevents either principal from reading both Secrets. |
| Supply-chain boundary | Partial repository lane | Pinned build/scanner inputs and reproducible Python artifacts are defined. Containers use Buildx `--provenance=false --load`; evidence then binds exact descriptor/manifest/config/loaded-image/Syft identities, emits unsigned repository SLSA, and verifies model-manifest/runtime/Dockerfile consistency. The always-run `evidence-integrity` job preserves all vulnerability findings while failing closed on stale/malformed/suppressive evidence, unexcepted secrets, and invalid or material-mismatched exceptions. A distinct `release-eligibility` job runs only for `v*` tags or explicit manual release evaluation, binds each container report to its exact Dockerfile SHA-256, and fails on unexcepted High/Critical findings or any pending external approval (**.github/workflows/supply-chain.yml; scripts/supply_chain_evidence.py; supply-chain/**). A green repository release job is only a precondition: no artifact is signed or promoted, and no bank registry, admission, digest-signing, or authenticated exception-approval control is proved. |
...
At the reviewed commit, the serving process could not operate with a read-only account. The current repository removes schema/corpus/cache-population/embedding initialization from `serve`; supplies separate schema-owner, ingestion, release-verifier, release-publisher, public, operator, and telemetry role/grant assets; and verifies exact effective privileges, ACL provenance, database/schema ownership, expected database, secure transport, and pool connections. Repository PostgreSQL tests prove the intended denial/allow matrix. The remaining gap is deployment evidence with the bank's seven actual LOGIN profiles, verifier/publisher Secret custody, DBA membership model, TLS/HBA policy, shared-cluster role names, and real upgrade data—not an absent repository role contract.
...
| corpus_release_verifier | read the exact corpus/legal evidence and stage an expiring state-bound release request; no activation or retention |
| corpus_publisher | activate one staged request by ID and retain its sealed generation; no corpus/evidence reads and no staging |
...
Repository upgrade documentation requires a restorable backup and size-matched rehearsal before dangerous migration approval. The current working tree adds a guarded populated-v2-to-current-schema rehearsal and recovery-evidence schema v2, including 53 managed objects, activation and staged-request bindings, exclusion of seven application DSNs, seven LOGIN profiles, live and retained-generation fingerprints, identity/catalog/readiness checks, and bounded PostgreSQL subprocess cleanup. Focused automated tests cover those contracts, and a retained local two-cluster PG17 execution proves the current schema-v8/53-object/seven-identity path with two request/bindings and two retained generations. Repository capability is not bank-accepted recovery evidence, so the following remain open:
...
- rollback/reactivation of a prior complete corpus image; v7 supplies sealed
  immutable targets, but serving is not generation-bound and no authorized
  activation/reactivation event path exists;
- key/config recovery;
...
- actual bank PostgreSQL LOGINs, memberships, verifier/publisher Secret custody, and ACL provenance; TLS/HBA policy; shared-cluster role names; database size; backups; and restore/upgrade results. Repository role, target-database, `verify-full`, per-connection identity, schema-v8 catalog, populated-v2-to-current rehearsal, and a retained local 53-object/seven-identity recovery execution exist, but no target-size TLS/HBA restore or PITR proof has run in the bank environment;
