...
corpus epoch and invalidates the active view. The current manifest is unsigned,
unquantified, and unmeasured, and its declared 8,286 chunks differ from the
9,675 rows regenerated by the current profile, so it intentionally fails strict
bootstrap and staging.
...
The current immutable ledger ends at schema v8. An ordinary v7 → v8 migration
...
publisher identity/catalog admission requires v8; a database left at v7 is an
upgrade source, not a compatible steady-state target for this release.
...
proxy-header trust is disabled, and the limiter does not accept client-supplied
`Forwarded` or `X-Forwarded-For` values. A bank deployment still needs shared
...
1. Provision a dedicated external PostgreSQL database, install `vector` and
   ...
   publication from non-owners, and current workload admission is v8-only.
