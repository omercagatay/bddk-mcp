...
- Migration v0005 persists an independently revalidated manifest/retrieval identity as an append-only release and activation. Mutation of any of 17 corpus tables advances a singleton epoch and invalidates the active view; strict local-corpus calls verify one release before and after execution. This is atomic activation and request consistency, not immutable retained generations or rollback. The tracked 8,286-chunk artifact currently differs from the 9,675 rows generated by profile v2, so the tracked corpus cannot pass strict publication.
