...
Varsayılan `public` process profili (`BDDK_TOOL_PROFILE=public` veya `bddk-mcp serve --profile public`) yalnızca 17 public araç expose eder.
...
| Düzenleyici graf | `get_amendment_chain`, `get_cross_references` |
...
Ayrı `operator` process profili (`BDDK_TOOL_PROFILE=operator` veya `bddk-mcp serve --profile operator`) 17 public araca 14 operatör aracı ekler ve toplam 31 araç expose eder. Bu profil ayrı, yazma yetkili `BDDK_OPERATOR_DATABASE_URL` ister; public DSN'e geri dönmez.
...
olmalıdır. Mevcut reviewed manifest (`bddk-job-corpus-2026-08-14`) Ed25519
imzalıdır ve numeric hedefler taşır (7 gün tespit, 14 gün yayın, 180 gün
manifest yaşı); `--require-quantified-freshness` ve
`--require-verified-signature` ile production bootstrap geçer. Canlı izleme
hattı olmadığı için `slo_evidence_status: not_measured` kalır ve
`--require-measured-freshness` isteyen verifier staging bilinçli olarak bu tek
kapıda reddeder. Başarılı bootstrap, operator evidence için path-free
...
Streamable HTTP MCP endpoint'i `http://localhost:8000/mcp` olur ve server stateless JSON response modunda çalışır. Remote uygulama RFC 9728 protected-resource metadata'yı `/.well-known/oauth-protected-resource/mcp` yolunda yayınlar; 401 challenge aynı URL'yi `resource_metadata` ile bildirir. Bu application-level MCP authorization discovery'dir; bank IdP client registration/flow acceptance kanıtı değildir. Sabit, content-free probe endpoint'leri `GET /health/live` ve `GET /health/ready`'dir; readiness migration, kritik catalog objeleri, corpus publication ve workload ACL'lerini periyodik olarak yeniden doğrular. Probe'lar authentication/Host kontrolü dışında olsa da process rate ve concurrency limitlerine tabidir. Loopback dışı bind fail-closed davranır: exact Host/HTTPS Origin allowlist'leri ve tam JWT/JWKS ayarları zorunludur; public profil `bddk.read`, operator profil `bddk.operator` scope'u ister. Remote operator ayrıca `BDDK_OPERATOR_REMOTE_ENABLED=true` ile açık opt-in ister. `BDDK_HTTP_ALLOW_UNAUTHENTICATED`, loopback dışı public read-only bind'i bearer authentication olmadan sunmak için desteklenen açık bir opt-in'dir; default olarak ayarlı değildir ve ayarlı değilken fail-closed default değişmez. Ayarlandığında herhangi bir `BDDK_JWT_` önekli ayarla birleştirilemez — startup reddeder ve ayarlı değişkenleri isimleriyle listeler — ve loopback dışı operator profili için `BDDK_OPERATOR_REMOTE_ENABLED` değerinden bağımsız olarak reddedilir; operator araçları authenticated veya loopback-only kalır. Unauthenticated server OAuth discovery yayınlamaz: `WWW-Authenticate` challenge yoktur ve her iki well-known OAuth route'u 404 döner. Host/Origin allowlist'leri ile body, concurrency ve rate limitleri uygulanmaya devam eder; bu modda rate limiter birincil abuse kontrolüdür. Rate limiter'ın client anahtarını `BDDK_HTTP_TRUSTED_PROXY_HOPS` (default `0`) belirler: `0` iken anahtar ASGI socket peer'ıdır ve `X-Forwarded-For` tamamen yok sayılır; operatörün kontrol ettiği `n` reverse proxy arkasında gerçek hop sayısı ayarlanır ve birleşik forwarded listesinin sağdan `n`. girdisi anahtar alınır. Kullanılamayan değerler socket peer'a düşmek yerine paylaşılan `unknown` bucket'ına düşer; yanlış bir değer limiter'ı ya paylaşılan ya da spoof edilebilir yapar. Body, concurrency ve dakikalık rate limitleri uygulama process'i içinde uygulanır; replica'lar arasında global ingress limiti sağlamaz. Ayrıntılar için [deployment belgesine](docs/DEPLOYMENT.md) bakın.
...
The default `public` process profile (`BDDK_TOOL_PROFILE=public` or `bddk-mcp serve --profile public`) exposes only 17 public tools.
...
| Regulatory graph | `get_amendment_chain`, `get_cross_references` |
...
The separate `operator` process profile (`BDDK_TOOL_PROFILE=operator` or `bddk-mcp serve --profile operator`) adds 14 operator tools to the 17 public tools and exposes 31 tools total. It requires a separate, write-capable `BDDK_OPERATOR_DATABASE_URL` and never falls back to the public DSN.
...
The Streamable HTTP MCP endpoint is `http://localhost:8000/mcp`, configured for stateless JSON responses. The remote application publishes RFC 9728 protected-resource metadata at `/.well-known/oauth-protected-resource/mcp`, and its 401 challenge identifies the same URL through `resource_metadata`. This is application-level MCP authorization discovery, not proof of bank IdP client registration or flow acceptance. The fixed, content-free probe endpoints are `GET /health/live` and `GET /health/ready`; readiness periodically re-attests migrations, critical catalog objects, corpus publication, and workload ACLs. Probes bypass authentication/Host checks but remain subject to process rate and concurrency admission. A non-loopback bind fails closed unless exact Host/HTTPS Origin allowlists and the complete JWT/JWKS configuration are supplied; the public profile requires `bddk.read`, while the operator profile requires `bddk.operator`. Remote operator HTTP also requires the explicit `BDDK_OPERATOR_REMOTE_ENABLED=true` opt-in. `BDDK_HTTP_ALLOW_UNAUTHENTICATED` is a supported explicit opt-in that serves a non-loopback public read-only bind without bearer authentication; it is unset by default, and while unset the fail-closed default is unchanged. When set, it cannot be combined with any `BDDK_JWT_`-prefixed setting — startup refuses and names the offending variables — and it is refused outright for a non-loopback operator profile regardless of `BDDK_OPERATOR_REMOTE_ENABLED`; operator tools stay authenticated or loopback-only. An unauthenticated server advertises no OAuth discovery: there is no `WWW-Authenticate` challenge, and both well-known OAuth routes return 404. Host/Origin allowlists plus the body, concurrency, and rate limits still apply, and the rate limiter becomes the primary abuse control. Its client key is governed by `BDDK_HTTP_TRUSTED_PROXY_HOPS` (default `0`): at `0` the limiter keys on the ASGI socket peer and ignores `X-Forwarded-For` entirely; behind `n` operator-controlled reverse proxies, set the real hop count so the key is the `n`-th entry from the right of the combined forwarded list. Anything unusable degrades to a shared `unknown` bucket rather than falling back to the socket peer; a wrong value makes the limiter either shared or spoofable. Body, concurrency, and per-minute rate controls are local to one application process and are not a shared ingress rate limit across replicas. See the [deployment guide](docs/DEPLOYMENT.md) for the complete contract.
